  ...
  <div class="nav">
    ...
    <div class="swiper-container nav-list">
        <ul class="swiper-wrapper item">
            <li class="swiper-slide"  v-for="(item,index) in navList" :key="index" :class="{'active': nowIndex == item.classID}" @click="tabClick(item.classID)"><a href="javascript:;">{{item.className}}</a></li>
        </ul>
    </div>
  </div>
</template>
...
import '../../../static/swiper.min.css';
  export default {
    ...
    created(){
      this.navList = this.$store.state.channelList;
    ...
    methods: {
      initPage() {
        ...
        for(var i=0;i<navList.length;i++){
          if(i==0){
            this.$store.dispatch('setChannelType',1);
          }else{
            if(navList[i].classID == this.common.getCaption(window.location.href)){
      ...
      getdetailpage(){
        ...
        if(operdir=='left'){
          ...
          for(var i=0;i<this.navList.length;i++){
            ...
            if((Number(nowChannelIndex)==this.navList[i].classID)&&(i<this.navList.length-1)){
              this.nowIndex = this.navList[i+1].classID;
            }
          }
          this.$store.dispatch('setChannelType',this.nowIndex);
        }else if(operdir=='right'){
          this.mySwiper.slidePrev();
          for(var i=0;i<this.navList.length;i++){
            nowChannelIndex = this.common.getCaption(window.location.href)
            if((Number(nowChannelIndex)==this.navList[i].classID)&&(i>0)){
              this.nowIndex = this.navList[i-1].classID;
            }
          }
          this.$store.dispatch('setChannelType',this.nowIndex);
